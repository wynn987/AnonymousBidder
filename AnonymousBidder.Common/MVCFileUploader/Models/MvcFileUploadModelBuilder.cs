...
namespace MvcFileUploader.Models
{
    ...
    public class MvcFileUploadModelBuilder : IMvcFileUploadModelBuilder
    {
        ...
        private dynamic GetUrlPostModel(bool isImages, bool IsCRM)
        {
            return new
            {
                FileTypes = _fileTypes,
        ...
        public IHtmlString RenderPopup(string labelTxt, string dataTarget, object htmlAttributes = null, bool isImages = true, bool IsCRM = false, string areaPrefix = "")
        {
            ...
            string linkUrl;
            linkUrl = urlHelper.Action("UploadDialog", "MvcFileUpload", GetUrlPostModel(isImages, IsCRM));
            if (!string.IsNullOrEmpty(areaPrefix))
            {
                var currentAreaPrefix = linkUrl.Substring(0, linkUrl.IndexOf("MvcFileUpload", StringComparison.OrdinalIgnoreCase));
                linkUrl = linkUrl.Replace(currentAreaPrefix, areaPrefix);
            }
            //else
            //{
            //    var param = new RouteValueDictionary(GetUrlPostModel(isImages));
            //    linkUrl = string.Format(areaPrefix + "/MvcFileUpload/UploadDialog?{0}", string.Join("&", param.Select(x => string.Format("{0}={1}", x.Key, x.Value))));
            //}
            ...
            if (_postValuesWithUpload.Count > 0)
            {
                ...
                {
                    linkUrl += String.Format("&postValues{1}.Key={0}", postVal.Key, "[" + idx + "]");
                    linkUrl += String.Format("&postValues{1}.Value={0}", postVal.Value, "[" + idx + "]");
                    idx++;
                }
            }
            else
            ...
            tag.Attributes.Add("id", dataTarget.Replace("#", "") + "-1");
