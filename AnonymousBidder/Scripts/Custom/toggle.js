...
function onDocReady() {
    ...
    $('.doc_menu_title').mousedown(function () {
        if ($(window).width() < 767) {
            $(this).toggleClass('active');
    ...
    $('.doc_content').mousedown(function () {
    ...
    if (testForIndex()) {
		...
		$.each(images,function(index, value)
		{
		  ...
		  if (formatCode) {
            ...
            $(".doc_menu a").each(function (index, value) {
            ...
            $.each(menus, function (index, value) {
                var menu = this;
                var anchors = $(menu).find('ul').find('a');
                var found = false;
            ...
            if (window.location.toString().indexOf("?search=") >= 0) {
                $("#searchField").val(window.location.toString().substring(window.location.toString().indexOf("?search=") + 8));
                filterMenuItems(false);
            }
            onDocReady();
            resize();

            if (window.location.href.indexOf("api.htm") >= 0 || window.location.href.indexOf("knockoutjs") >= 0 || window.location.href.indexOf("response") >= 0) {
                ...
                var args = $(".methodArgs");
                if (args.length > 0) {
                    var tables = args.find('table');
                    for (var j = 0; j < tables.length; j++) {
                        var hasDescription = false;
                        var table = tables[j];
                        for (var i = 0; i < table.rows.length; i++) {
                            var columns = $(table.rows[i]).find('td');
                            if (columns.length > 2) {
                                if (columns[2].innerHTML.length > 0) {
                                    hasDescription = true;
                                    break;
                                }
                            }
                        ...
                        if (!hasDescription) {
                            for (var i = 0; i < table.rows.length; i++) {
                                ...
                                var columns = $(table.rows[i]).find('th');
                                if (columns.length > 0) {
                                    $(columns[2]).hide();
                                }
                            }
                ...
                var input = $("<div style='width:40%; min-width:200px;' class='search'><div class='wrap'><input style='padding-left:10px; padding-right: 15px;' type='text' placeholder='I am searching for'/></div></div>");
                if (window.location.href.indexOf("response") >= 0 || window.location.href.indexOf("knockoutjs") >= 0) {
                    input.prependTo($("#properties"));
                }
                else {
                    input.prependTo(documentation);
                }
                ...
                input.keydown(function (event) {
                    if (timer != undefined) clearTimeout(timer);
                    timer = setTimeout(function () {
                        var searchString = input.find('input').val();
